...
pub async fn aim_trust_manifest(
    ...
    // Cache: aim_trust_manifest reads + SHA-256-hashes the whole .aim file and
    // spawns 2 git subprocesses. It's called ~3× per agent turn, which stalled
    // turns when the .aim file was large. Return a cached result when the file
    // is unchanged (mtime+size) and the entry is fresh (<3s) — makes repeat
    // calls instant.
    static TRUST_CACHE: OnceLock<Mutex<HashMap<String, (u128, Option<u128>, u64, Value)>>> = OnceLock::new();
    let cache_key = aim_path.to_string_lossy().to_string();
    let cur_mtime = file_modified_unix_ms(&aim_path);
    let cur_size = fs::metadata(&aim_path).map(|m| m.len()).unwrap_or(0);
    {
        let cache = TRUST_CACHE.get_or_init(|| Mutex::new(HashMap::new()));
        if let Ok(g) = cache.lock() {
            if let Some((computed_at, mt, sz, val)) = g.get(&cache_key) {
                if *mt == cur_mtime && *sz == cur_size && now_unix_ms().saturating_sub(*computed_at) < 3000 {
                    return Ok(val.clone());
                }
            }
        }
    ...
    if let Some(cache) = TRUST_CACHE.get() {
        if let Ok(mut g) = cache.lock() {
            g.insert(cache_key, (now_unix_ms(), cur_mtime, cur_size, manifest.clone()));
        }
    }
    Ok(manifest)
}
