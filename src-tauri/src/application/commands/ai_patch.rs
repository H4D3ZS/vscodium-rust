...
#[tauri::command]
pub async fn accept_sentient_patch(state: State<'_, std::sync::Arc<crate::EditorState>>, path: String) -> Result<(), String> {
    let mut engine = state.services.patch_engine.lock().await;
    engine.commit_shadow(std::path::Path::new(&path)).map_err(|e| e.to_string())
}

#[tauri::command]
    pub async fn reject_sentient_patch(state: State<'_, std::sync::Arc<crate::EditorState>>, path: String) -> Result<(), String> {
        let mut engine = state.services.patch_engine.lock().await;
