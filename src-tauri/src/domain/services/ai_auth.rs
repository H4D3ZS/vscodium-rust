...
#[tauri::command]
pub async fn get_api_keys(state: State<'_, std::sync::Arc<crate::EditorState>>) -> Result<ApiKeys, String> {
...
pub async fn save_api_keys(
    ...
    // hand-picked subset (openai/anthropic/google/lemonade/elevenlabs_api_key)
...
pub async fn hunt_api_keys(
    ...
    // apiradar.live leaked-key hunting is DISABLED — using third-party leaked API
    // keys is a legal/business liability and not appropriate for a shipped product.
    let _ = app.emit("hunt-progress", HuntProgress { msg: format!("Leak hunting disabled for {}.", provider) });
    let leaks: Vec<crate::hunter::ApiRadarLeak> = Vec::new();
