...
pub async fn start_emulator_stream(app: AppHandle, device_id: String) -> Result<String, String> {
    ...
    *STREAM_DEVICE.lock().unwrap() = Some(device.clone());
    ...
    let handle = tokio::spawn(async move {
        ...
        // Frame cadence adapts to available RAM. Every captured frame is a fresh
        // multi-MB PNG + a base64 String (+33%) crossing the IPC boundary, so on
        // a memory-starved box we deliberately drop the rate rather than pile on
        // allocations and push the machine toward swap/OOM. Recomputed every ~2s
        // (available_ram_gb spins up a sysinfo probe, so don't do it per frame).
        fn interval_for_ram(free_gb: f64) -> std::time::Duration {
            let ms = if free_gb < 1.5 {
                500 // ~2fps — barely-there box, keep it alive
            } else if free_gb < 3.0 {
                200 // ~5fps
            } else {
                100 // ~10fps
        ...
        let mut min_interval = interval_for_ram(crate::gpu_offload::available_ram_gb());
        let mut last_ram_check = std::time::Instant::now();
        let mut consecutive_failures = 0;

        while STREAM_RUNNING.load(Ordering::SeqCst) {
            let now = std::time::Instant::now();
            if now.duration_since(last_ram_check) >= std::time::Duration::from_secs(2) {
                min_interval = interval_for_ram(crate::gpu_offload::available_ram_gb());
                last_ram_check = now;
            }
