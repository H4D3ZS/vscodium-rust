...
mod mac {
    ...
    async fn start_embed_mirror(
        ...
        let open_fut = tokio::task::spawn_blocking({
            let u = udid.clone();
            move || crate::ios_sim_embed::open_simulator(&u)
        });
        let input_fut = {
            let u = udid.clone();
            tokio::task::spawn_blocking(move || spawn_input_helper(Some(&u)))
        };
    ...
    async fn start_stream_mirror(
        ...
        let capture_bin = tokio::task::spawn_blocking(ensure_sim_capture)
            .await
