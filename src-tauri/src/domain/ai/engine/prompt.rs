...
impl Sentient {
    ...
    /// Local backend bearer. Local Lemonade is keyless; a cloud Lemonade behind
    /// the JWT gate resolves through `get_key_for_provider`.
    pub(crate) fn local_bearer_for_base(&self, _base_url: &str) -> String {
        self.get_key_for_provider("lemonade")
    }
    ...
    pub(crate) fn get_key_for_provider(&self, provider: &str) -> String {
        ...
        // Lemonade: local (127.0.0.1) is keyless, but a CLOUD Lemonade behind the
        // Cyber-Ifrit JWT gate needs the signed-in subscription token. Resolve an
        // explicit `lemonade` key first, then fall back to the `cyberifrit` cloud
        // JWT (where `subscription_login` stores the session). Return empty (not the
        // generic api_key fallback) when nothing is set, so local stays keyless.
        if provider_base == "lemonade" {
            for env in ["LEMONADE_API_KEY", "CYBERIFRIT_API_KEY"] {
            ...
            if let Ok(content) = std::fs::read_to_string(&keys_path) {
                if let Ok(keys) = serde_json::from_str::<Value>(&content) {
                    for k in ["lemonade", "cyberifrit"] {
                        if let Some(v) = keys[k].as_str() {
        ...
        let env_var = match provider_base.as_str() {
            ...
            // OpenModel.ai — DeepSeek V4 Flash and other models.
            ...
            // Hugging Face Inference Router — uses HF_TOKEN or "huggingface" key.
            "huggingface" | "hf" => "HF_TOKEN",
            // ModelScope (Qwen Ambassador) — OpenAI-compatible cloud inference.
            "modelscope" | "qwen-ambassador" => "MODELSCOPE_API_KEY",
            _ => "OPENAI_API_KEY",
        };
        ...
        // (the save function may write to a different path than the engine reads from)
        let brain_keys = self.api_keys_path();
        let brain_parent = self.brain_parent();
        let fallback_keys = brain_parent.parent()
            .unwrap_or(brain_parent)
            .join("vscodium-rust").join("api_keys.json");
        for keys_path in [&brain_keys, &fallback_keys] {
            if let Ok(content) = std::fs::read_to_string(keys_path) {
                if let Ok(keys) = serde_json::from_str::<Value>(&content) {
    ...
    pub(crate) fn get_endpoint(&self, provider: &str, req: &AiRequest) -> String {
        ...
        match provider_base.as_str() {
            ...
            "lemonade" => {
                ...
                // Strip any endpoint suffix the user may have pasted, then
                // append the canonical chat path.
                let root = base
                    .trim_end_matches('/')
                ...
                format!("{}/api/v1/chat/completions", root)
            }
            // OpenModel.ai — Anthropic-compatible for DeepSeek models.
