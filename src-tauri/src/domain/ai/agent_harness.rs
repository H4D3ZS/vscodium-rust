...
/// Budget for 8-9B models with 16K context. System prompt ~4K tokens + tools ~3K
/// tokens leaves ~9K tokens ≈ 36K chars for conversation + results. Cap at 12K
/// chars to leave room for multi-turn history.
pub const SMALL_MODEL_TOOL_RESULT_CHAR_BUDGET: usize = 12_000;

/// Budget for 14B models with 24K context. More headroom for complex tasks.
pub const MEDIUM_MODEL_TOOL_RESULT_CHAR_BUDGET: usize = 20_000;
...
/// Compress old tool results for small models: keep only the last N full
/// results and summarize older ones into one-liners. This prevents the
/// conversation history from overwhelming a 16K context window.
pub fn compress_old_tool_results(messages: &mut Vec<ChatMessage>, keep_recent: usize) {
