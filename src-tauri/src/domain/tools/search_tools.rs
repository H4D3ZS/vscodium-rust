...
impl AiTools {
    pub(crate) async fn search_codebase(&self, args: Value) -> Result<Value> {
        ...
        for entry in walkdir::WalkDir::new(&root).max_depth(10).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file()) {
            let path = entry.path();
            if ignore_set.is_ignored(path) { continue; }
            if safe_io::should_skip_path(path) { continue; }

            // Use central safety limit for file size
            if let Ok(meta) = std::fs::metadata(path) {
                if meta.len() > safe_io::MAX_TEXT_FILE_SIZE / 2 { continue; }
            } else { continue; }
