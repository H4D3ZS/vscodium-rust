...
impl AiTools {
    ...
    pub(crate) async fn browser_screenshot(&self, _args: Value) -> Result<Value> {
    ...
    pub(crate) async fn browser_close(&self, _args: Value) -> Result<Value> {
        self.browser_state.stop_sidecar().await;
    ...
    pub async fn browser_subagent(self: Arc<Self>, args: Value) -> Result<Value> {
        ...
        let emit_progress = |step: &str, progress: u32, status: &str| {
            self.emit_tool_event("subagent-progress", json!({
        ...
        let _nav = self.browser_state.cmd("navigate", json!({ "url": format!("https://www.google.com/search?q={}", urlencoding::encode(&task)) }), 60).await
