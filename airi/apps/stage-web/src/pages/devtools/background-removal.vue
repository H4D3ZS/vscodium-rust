...
onMounted(async () => {
  try {
    ...
    // Configure transformers for better stability in iframes
    ...
    // Try WebGPU if supported, otherwise fallback to CPU
    let device = 'cpu'
    try {
      const gpuStatus = await check()
      if (gpuStatus.supported) {
        device = 'webgpu'
      }
    } catch (e) {
      console.warn('[BackgroundRemoval] WebGPU check failed, falling back to CPU:', e)
    }

    console.log(`[BackgroundRemoval] Loading model ${model_id} on ${device}...`)
    
    model.value ??= await AutoModel.from_pretrained(model_id, {
      device: device as any,
    }).catch(async (err) => {
      if (device === 'webgpu') {
        console.warn('[BackgroundRemoval] WebGPU loading failed, trying CPU fallback...', err)
        return await AutoModel.from_pretrained(model_id, { device: 'cpu' })
      }
      throw err
    })
    ...
    console.error('[BackgroundRemoval] Failed to initialize:', err)
    error.value = err
  }
