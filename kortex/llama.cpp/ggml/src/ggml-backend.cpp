...
// Initialize HADES Bridge (called once at first tensor operation)
static void ggml_backend_hades_ensure_init() {
    if (!g_hades_initialized) {
        // Check for cloud burst mode via environment
        const char* cloud_env = getenv("HADES_CLOUD_BURST");
        g_hades_cloud_burst = (cloud_env && (strcmp(cloud_env, "1") == 0 || strcasecmp(cloud_env, "true") == 0));
        
        if (hades_bridge_init()) {
            g_hades_initialized = true;
            fprintf(stderr, "[HADES] Bridge initialized - Mode: %s\n", 
                    g_hades_cloud_burst ? "192GB Cloud (paging bypass)" : "8GB Local (active paging)");
        }
    }
}

// Thermal throttle check - call every N tokens
static inline void ggml_backend_hades_maybe_throttle() {
    if (g_hades_initialized && !g_hades_cloud_burst) {
        g_hades_token_counter++;
        if (g_hades_token_counter >= 4) {  // Check every 4 tokens
...
struct ggml_backend_hades_layer_guard {
    ...
    ~ggml_backend_hades_layer_guard() {
        // Post-compute cleanup if needed
    }
};
#else
// Stub implementations when HADES is not enabled
static void ggml_backend_hades_ensure_init() {}
static void ggml_backend_hades_maybe_throttle() {}
#endif
