...
#ifdef HADES_BRIDGE_ENABLED
        // Check if HADES should handle mmap for zero-copy VRAM buffer
        const char* hades_env = getenv("HADES_MMAP_ENABLED");
        if (hades_env && (strcmp(hades_env, "1") == 0 || strcasecmp(hades_env, "true") == 0)) {
            // HADES will handle mmap via hades_mmap_vram_buffer
            // This is the injection point for custom mmap implementation
            LLAMA_LOG_INFO("[HADES] Custom mmap mode enabled - weights will use HADES RAII substrate\n");
        }
#endif
