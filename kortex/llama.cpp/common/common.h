...
struct common_hades_params {
    bool     enabled                     = false;   // Enable HADES Bridge
    bool     cloud_burst               = false;   // Bypass paging for 192GB cloud
    float    thermal_throttle_temp       = 72.0f; // Thermal throttle threshold (°C)
    int      thermal_check_interval      = 4;     // Check throttle every N tokens
    std::string model_path;                       // Path to GGUF model for weight streaming
};
