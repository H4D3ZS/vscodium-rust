...
export class AIRISelfLearning {
  ...
  private static readonly MAX_KNOWLEDGE_NODES = 500;
...
`;
  ...
  /**
   * Evict low-confidence knowledge nodes when the base exceeds the cap.
   * Removes the oldest entries first (FIFO eviction).
   */
  private evictLowConfidenceNodes(): void {
    if (this.knowledgeBase.size <= AIRISelfLearning.MAX_KNOWLEDGE_NODES) return;
    // Convert to array, sort by timestamp (oldest first), remove excess
    const entries = Array.from(this.knowledgeBase.entries())
      .sort((a, b) => (a[1].createdAt || 0) - (b[1].createdAt || 0));
    const toRemove = entries.slice(0, entries.length - AIRISelfLearning.MAX_KNOWLEDGE_NODES);
