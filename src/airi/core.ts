...
export class AIRICore {
  ...
  /**
   * Unsubscribe from AIRI events — prevents closure leaks.
   */
 ...
 console.warn('[AIRI] Ollama: DISCONNECTED (Failed to list models in non-Tauri env).');
  ...
  stop(): void {
    ...
    if (this.autonomousAgent) this.autonomousAgent.stop();
    this.selfHealing?.stop();
    airiConsciousness.suspend();
    airiPhaseWrap.stop();
    airiAmbitionSystem.stop();
    airiRelationshipMemory.stop();
    airiSecurity.stop();
    airiSelfLearning.stop();
    this.selfEvolution?.stop();
    this.senses?.stop();
    this.continuousImprovement?.stop();
    this.vision?.stop();
    airiSafetyProtocol.stop(true);
  }
