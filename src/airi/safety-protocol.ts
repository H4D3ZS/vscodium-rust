...
export class AIRISafetyProtocol {
    ...
    private static readonly MAX_THREAT_HISTORY = 200;
    ...
    private recordThreat(level: ThreatLevel, reason: string): void {
        ...
        if (this.threatHistory.length > AIRISafetyProtocol.MAX_THREAT_HISTORY) {
            this.threatHistory = this.threatHistory.slice(-AIRISafetyProtocol.MAX_THREAT_HISTORY);
        }
 ...
 console.error(' CRITICAL THREAT - INITIATING EMERGENCY SHUTDOWN ');
    ...
    removeShutdownCallback(callback: () => void): void {
        this.shutdownCallbacks.delete(callback);
    }

    private f12Handler: ((e: KeyboardEvent) => void) | null = null;
    ...
    private setupKeyboardShutdown(): void {
        ...
        if (this.f12Handler) return;

        this.f12Handler = (e: KeyboardEvent) => {
            if (e.key === 'F12') {
                this.initiateShutdown('F12 hardware kill-switch activated');
            }
        };
        window.addEventListener('keydown', this.f12Handler);
    }

    /**
     * Cleanup — removes the F12 handler to prevent OOM from stacking.
     */
    public cleanup(): void {
        if (this.f12Handler && typeof window !== 'undefined') {
            window.removeEventListener('keydown', this.f12Handler);
            this.f12Handler = null;
        }
    }
