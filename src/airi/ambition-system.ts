...
export class AIRIAmbitionSystem {
    ...
    private static readonly MAX_AMBITIONS = 20;
    ...
    start(): void {
        ...
        if (this.ambitionCheckInterval) return;
    ...
    /**
     * Stop ambition system — clears the interval to prevent OOM.
     */
    stop(): void {
        if (this.ambitionCheckInterval) {
            clearInterval(this.ambitionCheckInterval);
            this.ambitionCheckInterval = null;
        }
    }
    ...
    private generateInitialAmbitions(): void {
        ...
        // Cap ambitions to prevent unbounded growth
        if (this.ambitions.length > AIRIAmbitionSystem.MAX_AMBITIONS) {
            this.ambitions = this.ambitions.slice(-AIRIAmbitionSystem.MAX_AMBITIONS);
        }
    }
    ...
    startProject(project: LongTermProject): void {
        ...
        // Cap active projects to prevent unbounded growth
        if (this.activeProjects.length > AIRIAmbitionSystem.MAX_PROJECTS) {
            this.activeProjects = this.activeProjects.slice(-AIRIAmbitionSystem.MAX_PROJECTS);
        }
