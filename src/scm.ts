    ...
    msgInput.onkeydown = async (e) => {
        if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') {
            ...
            try {
                ...
                msgInput.value = '';
                await refreshScm();
                // Fire on_commit agent hooks (e.g. auto-generate changelog/tests).
                try { (window as any).useStore?.getState?.()?.fireHooks?.('on_commit', message.slice(0, 60)); } catch { /* non-fatal */ }
            } catch (err) {
