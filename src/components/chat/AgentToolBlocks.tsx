...
const EditDecision: React.FC<{ path?: string }> = ({ path }) => {
    ...
    return (
        <div className="agent-edit-actions" onClick={(e) => e.stopPropagation()} style={{ display: 'flex', gap: '6px' }}>
            <button className="ac-btn ac-btn--danger" onClick={reject} disabled={busy} title="Revert this file" style={{ padding: '3px 9px' }}>
                <i className="codicon codicon-discard" style={{ fontFamily: 'codicon', fontStyle: 'normal' }} />
                Reject
            </button>
            <button className="ac-btn ac-btn--ok" onClick={accept} disabled={busy} title="Keep this change" style={{ padding: '3px 9px' }}>
...
const cardStyle: React.CSSProperties = {
    ...
    border: '1px solid var(--ac-border, rgba(255,255,255,0.08))',
    background: 'var(--ac-surface, rgba(255,255,255,0.02))',
    overflow: 'hidden',
};
...
const headerStyle: React.CSSProperties = {
    ...
    padding: '6px 10px',
    fontSize: '12px',
    borderBottom: '1px solid var(--ac-border, rgba(255,255,255,0.06))',
    color: 'var(--ac-fg, rgba(255,255,255,0.85))',
};
...
const ToolBlockCard: React.FC<{ block: AgentToolBlock }> = ({ block }) => {
    ...
    const statusDot = (
        <span style={{
            ...
            background: isRunning ? 'var(--ac-accent, #3794ff)' : isError ? 'var(--ac-red, #f48771)' : 'var(--ac-fg-muted, rgba(255,255,255,0.25))',
