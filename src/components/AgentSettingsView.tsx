...
const AgentSettingsView: React.FC<AgentSettingsViewProps> = ({ category, hideHeader }) => {
    ...
    const plannerModel = useStore(state => state.plannerModel);
    const setPlannerModel = useStore(state => state.setPlannerModel);
    const plannerEnabled = useStore(state => state.plannerEnabled);
    const setPlannerEnabled = useStore(state => state.setPlannerEnabled);
    const hybridAuto = useStore(state => state.hybridAuto);
    const setHybridAuto = useStore(state => state.setHybridAuto);
        ...
        <div className="agent-settings-view" style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '20px', height: '100%', overflowY: 'auto' }}>
            {!hideHeader && !category && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '10px', padding: '20px 0', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <div style={{ fontSize: '18px', fontWeight: 800, color: 'var(--vscode-editor-foreground, #fff)', letterSpacing: '-0.5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        AIRI Neural Engine
                        <div style={{
                            padding: '2px 8px',
                ...
                <section data-cat="avatar">
                        ...
                        {avatarCharacters.map((char) => (
                            ...
                            >
                                <div style={{
                                    ...
                                    margin: '0 auto 6px',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                    fontSize: '14px', fontWeight: 800, color: 'var(--vscode-editor-foreground, #fff)',
                                }}>
                                    {char.name[0]}
                                </div>
                                <div style={{ fontSize: '10px', fontWeight: 600, color: 'var(--vscode-editor-foreground, #fff)' }}>{char.name}</div>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                            <div style={{ fontSize: '10px', fontWeight: 600, color: 'var(--vscode-editor-foreground, #fff)', textTransform: 'uppercase' }}>
            ...
            {visibleInCategory(category, 'models') && (
                <section data-cat="models">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <label style={{ fontSize: '11px', opacity: 0.8 }}>Active Model <span style={{ opacity: 0.6 }}>(executor — acts &amp; edits)</span></label>
                        <select
                            value={agentModel}
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginLeft: '22px' }}>
                                    ...
                                    {hybridAuto ? (
                                        ...
                                        <select
                                            ...
                                            style={{ background: 'var(--vscode-dropdown-background)', color: 'var(--vscode-dropdown-foreground)', border: '1px solid var(--vscode-dropdown-border)', padding: '4px', fontSize: '12px', cursor: 'pointer' }}
                                        >
                                            <option value="">— select planner model —</option>
                                            {availableModels.map(m => (
                                                <option key={`plan-${m.provider}|${m.id}`} value={modelKey(m as any)}>
                                                    {m.provider.toUpperCase()} - {m.id}
                                                </option>
                                            ))}
                                        </select>
                    ...
                    {isAddingMcp && (
                        <div style={{ marginBottom: '16px', padding: '10px', background: 'var(--vscode-sideBar-background)', border: '1px solid var(--vscode-panel-border)', borderRadius: '2px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            ...
                            {newMcpType === 'command' ? (
                                <>
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        style={{ background: 'rgba(0,0,0,0.2)', color: 'var(--vscode-editor-foreground, #fff)', border: '1px solid rgba(255,255,255,0.1)', padding: '4px 8px', fontSize: '11px', borderRadius: '4px', fontFamily: 'var(--font-mono)', resize: 'vertical' }}
                                    />
                                    <input
                            ...
                            <button
                                ...
                                style={{ background: '#3b82f6', color: 'var(--vscode-editor-foreground, #fff)', border: 'none', padding: '6px', fontSize: '11px', borderRadius: '4px', cursor: 'pointer', fontWeight: 600 }}
                            >
                                Connect Server
                            </button>
                        </div>
