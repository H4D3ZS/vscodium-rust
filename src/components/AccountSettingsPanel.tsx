...
const AccountSettingsPanel: React.FC = () => {
    ...
    const refresh = useCallback(() => {
        ...
        invoke<AccountView>('account_get').then((d) => {
            ...
            // NOTE: do NOT dispatch 'account:changed' here — refresh() also listens
            // for it, which created a refresh→dispatch→refresh loop (the ToS flicker).
            // We notify the status-bar chip only after real mutations (notifyChip()).
        }).catch(() => {});
        invoke<{ used_tokens: number; limit_tokens: number; used_month: number; limit_month: number }>('account_usage').then(setUsage).catch(() => {});
        // ToS acceptance is monotonic — never un-accept on a later read (guards
        // against best-effort backend reads returning false after a local accept).
        invoke<boolean>('account_tos_status', { docId: BUG_BOUNTY_TOS_ID })
            .then((v) => setTosAccepted((prev) => prev || v)).catch(() => {});
    }, []);
    // Notify the status-bar chip after a real account change (not on every refresh).
    const notifyChip = useCallback(() => { window.dispatchEvent(new Event('account:changed')); }, []);
    ...
    const acceptTos = () => {
        // Optimistic: set accepted immediately so the UI doesn't flicker while the
        // backend round-trips (esp. when Supabase isn't configured and status reads
        // are best-effort).
        setTosAccepted(true);
        setShowTos(false);
        invoke('account_accept_tos', { docId: BUG_BOUNTY_TOS_ID, version: BUG_BOUNTY_TOS_VERSION })
            .then(() => { refresh(); notifyChip(); }).catch(() => {});
    };
